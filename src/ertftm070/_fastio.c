...
/* Bus pins (BCM numbering) — must match ertftm070.pins.DEFAULT_PINS. */
static const int db_low[8] = {4, 17, 27, 22, 5, 6, 13, 19};   /* DB0..DB7 */
static const int db_high[8] = {7, 8, 9, 10, 11, 18, 23, 24};  /* DB8..DB15 */
#define WR_PIN 21
#define RD_PIN 16
...
/* Precomputed masks for the 16 data pins. */
...
/* Set pin `pin` (0..53) to output (mode=1) or input (mode=0).
...
static void bus_write_byte(uint8_t value)
{
    ...
    gpio[GPSET0] = 1u << WR_PIN;     /* WR high: byte latched */
    spin();                          /* inter-byte gap */
}
...
static uint32_t bus_read_word(void)
{
    ...
    gpio[GPCLR0] = 1u << RD_PIN;     /* RD low: sample window */
    nanosleep(&tw, NULL);
    lev = gpio[GPLEV0];
    gpio[GPSET0] = 1u << RD_PIN;     /* RD high */
    for (b = 0; b < 16; b++) {
...
static PyObject *py_open(PyObject *self, PyObject *noargs)
{
    int fd;
    void *map;
...
static PyObject *py_pin_mode(PyObject *self, PyObject *args)
{
    ...
    if (pin < 0 || pin >= GPIO_COUNT) {
        PyErr_SetString(PyExc_ValueError, "pin out of range 0..53");
        return NULL;
    }
...
static PyObject *py_pixel_stream(PyObject *self, PyObject *arg)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        spin();
        gpio[GPCLR0] = 1u << WR_PIN;
        spin();
        gpio[GPSET0] = 1u << WR_PIN;
        spin();
    }
...
static PyMethodDef methods[] = {
    {"open", py_open, METH_NOARGS,
     "open() — open /dev/gpiomem and map the GPIO registers."},
